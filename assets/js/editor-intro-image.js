(function (window, wp) {
  ...
  var defaultStrings = {
    ...
    panelDescription: __(
      "Select an image to replace the intro highlight. It behaves like the custom header artwork.",
      "smile-web"
    ),
    ...
    function IntroImagePanel() {
      ...
      useEffect(
        function () {
          ...
          wp.apiFetch({
            path: "/wp/v2/media/" + introImageId + "?context=edit",
          })
      ...
      if (mediaDetails) {
        if (mediaDetails.media_details && mediaDetails.media_details.sizes) {
          if (
            mediaDetails.media_details.sizes.large &&
            ...
            mediaDetails.media_details.sizes.medium_large.source_url
          ) {
            previewUrl =
              mediaDetails.media_details.sizes.medium_large.source_url;
          } else if (
            mediaDetails.media_details.sizes.full &&
            mediaDetails.media_details.sizes.full.source_url
          ) {
          ...
          createElement(MediaUpload, {
            ...
            render: function (props) {
              return createElement(
                ...
                introImageId
                  ? getString("replaceImageButton")
                  : getString("selectImageButton")
              );
            },
          })
        ),
